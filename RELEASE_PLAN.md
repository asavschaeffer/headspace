...
The naming/current-format release candidate is committed and clean. Release operations still required: connect the intended Git remote and let the Windows and Ubuntu CI jobs pass from that exact commit. A stale local annotated `v0.1.0` tag that pointed to `233b7c1`, the parent of the naming refactor, was removed after GitHub verification confirmed that `asavs/headspace` has no remote tags. Its target commit remains intact. Recreate `v0.1.0` only after CI passes from the final release commit.

No remote or upstream is currently configured. The public repository's `main` remains at its November 2025 Cosmic Diary history, while this repository's current `main` begins from a separate root; Git reports no merge base between them. Publishing is therefore an explicit repository-history decision, not a routine fast-forward push. Do not attach, merge, replace, or force-update the public repository by inference.
