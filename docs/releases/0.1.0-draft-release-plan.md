# Headspace 0.1.0 release workback — future draft

> **Future draft, not the current release plan.** This document preserves the broader 0.1.0 workback. Headspace 0.0.1 is the current, intentionally narrow text-kernel preview; see the root [release plan](../../RELEASE_PLAN.md).

> **Audit note, 2026-08-24.** Loop 6 was reopened when a product-level review found that pending browser commits are memory-only and the release-runtime test does not drive the built UI. Before 0.1.0, either persist that queue or narrow the promise, add a real built-browser loop proof, and recheck release-note claims about graph-link visibility and on-demand decomposition. The actionable Markdown projection diagnostic was pulled forward into 0.0.1.

This is a living implementation plan derived backward from the [0.1.0 draft release notes](0.1.0-draft-release-notes.md). The release notes describe the promised experience; this document records what must become true to make that promise honest.
...
Status: reopened; browser durability and real built-UI proof pending
