...
// difference, so a pair that far apart provably cannot reach the adapter's 0.5
...
// dev server runs with every other request queued behind it.
{
  ...
  const editedTail = `${huge.slice(0, huge.length - 40)}tail rewritten outside of headspace xx`;
...
const root = mkdtempSync(join(tmpdir(), 'headspace-md-'));
try {
  ...
  const assertNoAtomicTemps = (path: string): void => {
    assert.deepEqual(
      readdirSync(dirname(path)).filter((name) => name.startsWith('.headspace-write-')),
      [],
  ...
  const imported = await importMarkdownFile(filesystemAdapter, {
    workspaceRoot: root,
    relPath: rel,
    text: md1,
  ...
  assert.equal(imported.commitId, imported.commit.id, 'adapter returns the exact persisted import commit');
  assert.equal(imported.operationId, imported.commit.operation.id, 'adapter returns the exact persisted import operation');
  ...
  const unsupportedSidecar = structuredClone(sc1) as { schemaVersion?: number };
  delete unsupportedSidecar.schemaVersion;
  writeFileSync(sidecarPath(root, rel), `${JSON.stringify(unsupportedSidecar, null, 2)}\n`);
  await assert.rejects(
    reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md1 }),
    /unsupported Markdown sidecar schema.*expected 2/,
  );
  writeFileSync(sidecarPath(root, rel), `${JSON.stringify(sc1, null, 2)}\n`);

  // unchanged file → noop
  assert.equal((await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md1 })).action, 'noop');
  ...
  const r2 = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md2 });
  ...
  blockChunkIds.forEach((id, i) => {
    ...
    if (i === 1) {
      ...
      assert.equal(state.revisions.get(now)!.createdBy, 'adapter:filesystem');
    } else {
  ...
  const r2b = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md2b });
  ...
  const md3 = mk([A, B2, D]);
  const r3 = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md3 });
  ...
  const r4 = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: md4 });
  ...
  assert.equal(state.revisions.get(bRevNow)!.createdBy, 'adapter:filesystem', 'external text attributed to the adapter');
  ...
  await writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: rel });
  const onDisk = readFileSync(relAbs, 'utf8');
  assert.equal(onDisk, projectMarkdown(state, docChunkId));
  assert.equal((await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: rel, text: onDisk })).action, 'noop');
  ...
  await assert.rejects(
    writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: rel }),
  ...
  await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: missingRel, text: '# Missing\n' });
  const missingSidecarBefore = readFileSync(sidecarPath(root, missingRel), 'utf8');
  await assert.rejects(
    writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: missingRel }),
  ...
  const recovery = await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: recoveryRel, text: recoveryText });
  ...
  await writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: recoveryRel });
  ...
  const extractedDoc = await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: extractRel, text: extractText });
  ...
  await writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: extractRel });
  ...
  const flattenResult = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: extractRel, text: flattenedText });
  ...
  assert.equal(
    (await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: extractRel, text: flattenedText })).action,
    'fast-forward',
  ...
  const sourceDoc = await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: sourceRel, text: sourceText });
  const targetDoc = await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: targetRel, text: targetText });
  ...
  const targetProjection = await writeProjection(filesystemAdapter, { workspaceRoot: root, relPath: targetRel });
  ...
  const detachResult = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: targetRel, text: detachedText });
  ...
  assert.equal(
    (await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: targetRel, text: detachedText })).action,
    'fast-forward',
  ...
  // by the adapter — it is proposed and waits for a human.
  const mixRel = 'notes/mixed.md';
  const P1 = '# Mixed';
  const P2 = 'alpha paragraph';
  const P3 = 'beta paragraph';
  const P4 = 'gamma paragraph';
  const mixed = await importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: mixRel, text: mk([P1, P2, P3, P4]) });
  ...
  const mixResult = await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: mixRel, text: mixedText });
  ...
  assert.equal(state.revisions.get(currentRevision(state, c2).id)!.createdBy, 'adapter:filesystem');
  ...
  assert.equal(
    (await reconcileMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: mixRel, text: mixedText })).action,
    'noop',
    'the round trip settles',
  );

  // path traversal rejected before anything is written
  await assert.rejects(importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: '../escape.md', text: 'x\n' }));
  await assert.rejects(importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: 'a/../../escape.md', text: 'x\n' }));
  await assert.rejects(importMarkdownFile(filesystemAdapter, { workspaceRoot: root, relPath: resolve(root, '..', 'abs.md'), text: 'x\n' }));
  assert.throws(() => sidecarPath(root, '..'));
  assert.throws(() => sidecarPath(root, '.'));

  console.log('markdown adapter OK —', state.chunks.size, 'chunks,', state.proposals.size, 'proposals');
} finally {
